...
if st.session_state.analyzed and 'results' in st.session_state:
    ...
    with col_chart1:
        ...
        st.write(f"**CGPA Score:** {results['score_breakdown']['cgpa_score']:.1f}/40")
        st.write(f"**Skills Score:** {results['score_breakdown']['skills_score']:.1f}/30")
        st.write(f"**Projects Score:** {results['score_breakdown']['projects_score']:.1f}/20")
        st.write(f"**Key Skills Bonus:** {results['score_breakdown']['key_skills_bonus']:.1f}/10")
    
    with col_chart2:
        st.subheader("🏢 Company-wise Predictions")
        for company, prob in results['company_predictions'].items():
